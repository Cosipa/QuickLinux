...
"@
        ...
        # Max available stored for the disk plan dialog (conservative estimate; exact boot size computed later from ISO)
        $script:MaxAvailableGB = [math]::Floor($script:CDriveInfo.FreeGB - 12 - 10)
    }
    catch {
...
function Show-DiskPlan {
    ...
    if ($distro.Custom -and $script:CustomIsoPath -and (Test-Path $script:CustomIsoPath)) {
        $isoSizeGB = (Get-Item $script:CustomIsoPath).Length / 1GB
    } elseif ($distro.IsoFilename) {
        $cachedIso = Join-Path $env:TEMP $distro.IsoFilename
        if (Test-Path $cachedIso) {
            $isoSizeGB = (Get-Item $cachedIso).Length / 1GB
        } elseif ($distro.SizeGB) {
            $isoSizeGB = $distro.SizeGB
        } else {
            $isoSizeGB = 6
        }
    } else {
        ...
        if ($isTargetCDisk) {
            ...
            switch ($strategy) {
                "shrink_all" {
                    ...
                    $changeLines += "  2. Create $bootPartSizeGB GB FAT32 boot partition (LINUX_LIVE) with $distroName files"
                ...
                "use_free_all" {
                    ...
                    $changeLines += "  2. Create $bootPartSizeGB GB FAT32 boot partition (LINUX_LIVE) in existing free space"
            ...
            if ($usingWipe) {
                ...
                $changeLines += "  3. Create $bootPartSizeGB GB FAT32 boot partition (LINUX_LIVE) on Disk $selDiskNum"
                ...
                if ($hasFreeSpace) {
                    $changeLines += "  1. C: partition is NOT modified (different disk selected)"
                    $changeLines += "  2. Create $bootPartSizeGB GB FAT32 boot partition (LINUX_LIVE) on Disk $selDiskNum"
...
function Start-Installation {
    ...
    $bootPartSizeGB = $planResult.BootPartSizeGB
    $totalNeededGB = $linuxSizeGB + $bootPartSizeGB
    Log-Message "Disk plan approved. Strategy: $selectedStrategy, Target disk: $targetDiskNumber, Linux size: $linuxSizeGB GB, Boot partition: $bootPartSizeGB GB"
    ...
    try {
        ...
        if ($selectedStrategy -eq "shrink_all") {
            ...
            if ($otherDiskFreeGB -lt ($bootPartSizeGB + 1)) {
                Log-Message "Error: Not enough unallocated space on Disk $targetDiskNumber!" -Error
                Log-Message "Need: $($bootPartSizeGB + 1) GB, Have: $otherDiskFreeGB GB" -Error
                return
            }
...
"@
            ...
            Log-Message "  Partition 1: LINUX_LIVE ($bootPartSizeGB GB, ${driveLetter}:)"
        ...
        elseif ($selectedStrategy -eq "other_drive_shrink") {
            ...
            Log-Message "This will create space for Linux: $linuxSizeGB GB and boot partition: $bootPartSizeGB GB..."
            ...
            if ($selectedStrategy -eq "shrink_all") {
                ...
                Log-Message "The $bootPartSizeGB GB boot partition will use existing unallocated space."
            }
        ...
        Set-Status "Creating boot partition..."
        Log-Message "Creating $bootPartSizeGB GB boot partition on Disk $targetDiskNumber..."
...
- Boot Drive: $script:NewDrive ($bootPartSizeGB GB, FAT32, labelled LINUX_LIVE)
